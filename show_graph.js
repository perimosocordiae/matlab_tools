(function($){
  var Renderer = function(canvas_id){
	...
	particleSystem.eachNode(function(node, pt){
          ...
          // clear area for label
          ctx.fillRect(pt.x-w/2, pt.y-7, w, 14);
          // bounding box for label
          ctx.strokeStyle = node.data.color || "black";
          ctx.strokeRect(pt.x-w/2, pt.y-7, w, 14);
  ...
  $(document).ready(function(){
    ...
    //XXX: hack
    sys.merge({nodes:graph.nodes, edges:graph.edges});

	$('#pause').click(function(){
    if ($('#pause').text().match(/Pause/i)){
      $('#pause').text('Resume Graph Layout');
        sys.stop();
      } else {
        $('#pause').text('Pause Graph Layout');
        sys.start();
