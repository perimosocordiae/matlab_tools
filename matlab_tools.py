...
def parse_args():
  ...
  p.add_argument('--graph', default='-', help='generate a project dependency graph')
...
def find_usages(functions, files, targets):
  ...
  for fn,fpath in functions.iteritems():
    ...
    for j,fpath2 in enumerate(files.itervalues()):
      if fpath not in targets and fpath2 not in targets: continue
